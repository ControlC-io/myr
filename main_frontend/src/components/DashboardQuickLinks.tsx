...
const DashboardCard = ({ link, label, onClick }: CardProps) => {
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="relative group flex flex-col items-center justify-center gap-3 p-5 rounded-xl
    ...
    >
      {/* Badge */}
      {link.badge && (
        <span className={`absolute top-2.5 right-2.5 px-1.5 py-0.5 rounded-full text-[10px] font-bold leading-none ${
